...
def lstm_train(X_train, X_val, y_train, y_val, params):
    ...
    save_tokenizer = experiment.get_outputs_path() + '/LSTM_tokenizer.pickle'
    ...
    if params['bidirectional'] == True:
        model.add(Bidirectional(LSTM(units=params['units'])))
    else:
    ...
    experiment = Experiment()
    saved_path = experiment.get_outputs_path() + '/LSTM_coronatweetmodel.h5'
